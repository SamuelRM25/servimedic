...
try {
    ...
    $stmt = $conn->prepare("
        SELECT e.*, u.nombre as usuario_nombre, s.nombre as sucursal_nombre
        FROM examenes e
...
<head>
    ...
    <title>Exámenes - Servimedic</title>
    ...
    <div class="container">
        ...
        <div class="stats-grid">
            <div class="stat-card">
                ...
                <div class="stat-value"><?php echo count($examenes); ?></div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Pacientes Privados</div>
                <div class="stat-value">
                    <?php echo count(array_filter($examenes, fn($e) => $e['tipo_paciente'] === 'Privado')); ?>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Pacientes EPS</div>
                <div class="stat-value">
                    <?php echo count(array_filter($examenes, fn($e) => $e['tipo_paciente'] === 'EPS')); ?>
                </div>
            </div>
            <div class="stat-card">
                <div class="stat-label">Total Cobrado</div>
                <div class="stat-value">
                    Q<?php echo number_format(array_sum(array_column($examenes, 'monto')), 2); ?>
                </div>
            </div>
        </div>
        ...
        <!-- Form -->
        <div class="form-card">
            <h3 class="form-section-title">Registrar Nuevo Examen</h3>
                ...
                <div class="form-group">
                    ...
                    <div class="checkbox-list">
                        <label class="checkbox-item">
                            <input type="checkbox" name="examenes[]" value="Electrocardiograma (ECG)">
                            <span>Electrocardiograma (ECG)</span>
                        </label>
                        <label class="checkbox-item">
                            <input type="checkbox" name="examenes[]" value="Ultrasonido">
                            <span>Ultrasonido</span>
                        </label>
                        <label class="checkbox-item">
                            <input type="checkbox" name="examenes[]" value="Radiografía">
                            <span>Radiografía</span>
                        </label>
                        <label class="checkbox-item">
                            <input type="checkbox" name="examenes[]" value="Examen General de Orina">
                            <span>Examen General de Orina</span>
                        </label>
                        <label class="checkbox-item">
                            <input type="checkbox" name="examenes[]" value="Hematología Completa">
                            <span>Hematología Completa</span>
                        </label>
                        <label class="checkbox-item">
                            <input type="checkbox" name="examenes[]" value="Papanicolaou">
                            <span>Papanicolaou</span>
                        </label>
                    </div>
                    <div style="margin-top: 1rem;">
                        <input type="text" name="otro_examen" class="form-control" placeholder="Otro examen (especifique)...">
                    </div>
                </div>
        ...
        <div class="table-container">
            <div style="padding: 1.5rem; border-bottom: 1px solid var(--color-border);">
                <h3 style="margin: 0; font-size: 1.125rem; font-weight: 600;">Historial de Exámenes</h3>
            </div>
            <div class="table-wrapper">
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td><strong>#<?php echo str_pad($examen['id_examen'], 6, '0', STR_PAD_LEFT); ?></strong></td>
                            <td><?php echo date('d/m/Y H:i', strtotime($examen['fecha_examen'])); ?></td>
                            <td><?php echo htmlspecialchars($examen['nombre_paciente']); ?></td>
                            <td>
                                <span class="badge badge-<?php echo strtolower($examen['tipo_paciente']); ?>">
                                    <?php echo $examen['tipo_paciente']; ?>
                                </span>
                            </td>
                            <td><small><?php echo htmlspecialchars($examen['examenes_realizados']); ?></small></td>
                            <td>
                                <?php 
                                $pagoClass = 'badge-privado';
                                if ($examen['tipo_pago'] === 'Seguro Manual') $pagoClass = 'badge-seguro';
                                if ($examen['tipo_pago'] === 'EPS') $pagoClass = 'badge-eps';
                                ?>
                                <span class="badge <?php echo $pagoClass; ?>">
                                    <?php echo $examen['tipo_pago']; ?>
                                </span>
                            </td>
                            <td><?php echo $examen['metodo_pago']; ?></td>
                            <td><strong>Q<?php echo number_format($examen['monto'], 2); ?></strong></td>
                            <td>
                                <button onclick='printTicket({
                                    ticketNumber: "<?php echo $examen["numero_ticket"] ?? str_pad($examen["id_examen"], 6, "0", STR_PAD_LEFT); ?>",
                                    date: "<?php echo date("d/m/Y H:i", strtotime($examen["fecha_examen"])); ?>",
                                    patientName: "<?php echo htmlspecialchars($examen["nombre_paciente"]); ?>",
                                    items: [{
                                        quantity: 1,
                                        description: "<?php echo htmlspecialchars($examen["examenes_realizados"]); ?>",
                                        total: <?php echo $examen["monto"]; ?>
                                    }],
                                    total: <?php echo $examen["monto"]; ?>
                                })' 
